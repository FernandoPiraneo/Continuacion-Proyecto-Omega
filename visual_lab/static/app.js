...
function renderScannerPanels() {
  ...
  const primaryTrade = visualState?.primary_trade || null;
  ...
  if (plan) {
    if (!fractalState && !fractalTree) {
      ...
      const setupRows = ["15m", "5m", "3m", "1m"]
        .map((tf) => {
          const setup = tradeSetupsByTf[tf] || {};
...
function renderPivotMarkers(visualState, selectedTf) {
  ...
  const lastPoints = points.slice(-80);
  const markers = [];
  ...
  for (const point of lastPoints) {
    ...
    markers.push({
      ...
      color: kind === "HIGH" ? "#ef4444" : "#22c55e",
      shape: kind === "HIGH" ? "arrowDown" : "arrowUp",
      text: kind === "HIGH" ? "H" : "L",
    });
  }
...
function renderEmaCrossMarkers(visualState, selectedTf) {
  const crosses = Array.isArray(visualState?.ema_crosses_by_tf?.[selectedTf])
    ? visualState.ema_crosses_by_tf[selectedTf].slice(-100)
    : [];
...
function renderSetupLevelOverlays(visualState, selectedTf) {
  ...
  const setup = visualState?.trade_setups_by_tf?.[selectedTf] || {};
  const primary = visualState?.primary_trade || {};

  addSetupPriceLine(setup.entry, { color: "#38bdf8", title: `Entry ${selectedTf}` });
  addSetupPriceLine(setup.stop_loss, { color: "#ef4444", title: `SL ${selectedTf}` });
  addSetupPriceLine(setup.tp1, { color: "#22c55e", title: `TP1 ${selectedTf}` });
  addSetupPriceLine(setup.tp2, { color: "#16a34a", title: `TP2 ${selectedTf}` });
  addSetupPriceLine(setup.tp3, { color: "#15803d", title: `TP3 ${selectedTf}` });
